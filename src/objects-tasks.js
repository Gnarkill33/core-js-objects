...
function shallowCopy(obj) {
  const newObject = {};
  const newCopy = Object.assign(newObject, obj);
  return newCopy;
}
...
function makeWord(lettersObject) {
  ...
  const valueArray = Object.values(lettersObject);
  const entryArray = Object.entries(lettersObject);
  const maxIndex = Math.max(...valueArray.flat());
...
function sellTickets(queue) {
  ...
  queue.forEach((bill) => {
    if (bill === 25) {
      ...
      changeFund = changeFund - change + 25;
    }
  });
  return changeFund >= 0;
}
